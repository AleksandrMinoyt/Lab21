    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 15; i++)
            {
                ...
                {
                    int maxVal = j * 16; 
                    array[i, j] = (byte)rnd.Next(4, 4+maxVal);  // время на обработку 1 участка, чем праве, тем дольше...
                }
        ...
        static void gardener2()
        {
            for (int j = 14; j >= 0; j--)
            {
                for (int i = 14; i >= 0; i--)
                {
                    ...
                    Thread.Sleep(del*3); // второй садовник начинающий, ему нужно в 3 раза больше времени
                }
            }
            lock (lck)
            {
